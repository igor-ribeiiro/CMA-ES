...
O objetivo deste trabalho é desenvolver e implementar um algoritmo de otimização de parâmetros para um time de futebol composto por três robôs diferenciais. O contexto é uma partida de futebol segundo a Competição Brasileira de Robótica (CBR), categoria IEEE Very Small Size Soccer (VSSS). À partir de simulações computacionais com métricas de análise de desempenho e, por meio de algoritmos evolutivos, deve-se encontrar os melhores parâmetros para diversas áreas de conhecimento utilizadas pelo time, em especial o Univector, que é o planejamento de trajetórias usado. Visa-se utilizar os algoritmos aqui implementados pela equipe da ITAndroids, que representa o ITA, em competições nacionais e internacionais, como na competição Latin America Robotics Competition (LARC)/CBR.

Planejamento:
\begin{itemize}

\item 1o Bimestre (ago / set): Estudo da literatura e revisão bibliográfica sobre o tema de otimização de parâmetros para robôs jogadores de futebol. Seleção dos métodos a serem implementados.

\item 2o Bimestre (out / nov): Implementação inicial do modelo estrutural do projeto e definição das métricas a serem usadas para análise de desempenho dos times.

\item 3o Bimestre (dez / jan): Implementação dos testes e simulações para as métricas escolhidas. Redação do relatório parcial.

\item 4o Bimestre (fev / mar): Implementação e análise dos algoritmos escolhidos para otimização. Documentação do processo.

\item 5o Bimestre (abr / mai): Análise do desempenho da otimização em simulações. Ajuste do algoritmo para se obter melhores resultados.
...
Ao longo dos dois primeiros meses, foram realizados estudos e foi decidido que será usado o algoritmo Covariance Matrix Adaptation Evolution Strategy (CMA-ES) para a otimização de parâmetros. Esse algoritmo foi escolhido dentre opções como Downhill Simplex Method e Broyden–Fletcher–Goldfarb–Shanno Algorithm. Uma introdução a essas outras opções pode ser encontrada, respectivamente, em \cite{koshel2002enhancement} e em \cite{broyden1973local}. Uma descrição sobre a eficiência do método CMA-ES aplicado em otimização de parâmetros encontra-se em \cite{hansen2003reducing}.

Ao longo do segundo e terceiro bimestres, a base do algoritmo foi implementada utilizando Matlab\textsuperscript{\textregistered}, linguagem alto nível para operações de Álgebra Linear. Foi, em seguida, criado as métricas e os testes simulacionais para avaliar o desempenho de um determinado conjunto de parâmetros. A performance computacional do CMA-ES foi analisada e se mostrou condizente com o que foi previsto. A estrutura básica do algoritmo estava pronta.

Os parâmetros encontrados pelo algoritmo foram implementados e testados no time completo de robôs. O método  foi mostrado efetivo na LARC, ocorrida entre os dias 6 a 10 de novembro de 2018. O algoritmo mostrou que os parâmetros obtidos, em especial os do planejamento de trajetória, fizeram que os robôs jogassem de forma mais fluída e consistente, de modo que a equipe tinha um dos ataques mais fortes da competição. A equipe da ITAndroids conquistou o XXXXXX lugar de 30 competidores.

\section{Descrição do Problema}
\label{secao:enunciado_problema}

No contexto do futebol de robôs, o ajuste de parâmetros traz muitos desafios. A otimização consiste no fato de se implementar um algoritmo que encontre parâmetros desejados de forma automática e em tempo hábil, já que as simulações usadas são computacionamente custosas. A figura \ref{fig:vss_ele} mostra a placa eletrônica do robô utilizado. 
...
A figura \ref{fig:vss} mostra os robôs usados nas competições. A visão computacional utilizada pela equipe ITAndroids pode ser encontrada com mais detalhes em \cite{zickler2009ssl} e utiliza uma câmera no topo do campo que repassa informaçãoes para um computador processar, como visto na figura \ref{fig:funcioamento}.
...
Cada robô foi projetado com duas rodas laterais e duas esferas livres à frente e atrás para manter o equilíbrio. Essa característica o torna um robô diferencial. Ou seja, temos o controle sobre as velocidades linear e angular, mas não é possível mover o robô para os lados. A linguagem utilizada para o projeto foi C++, pois é uma linguagem que tem uma velocidade de execução alta e tem escalabilidade descente para um projeto grande, como estudado em \cite{cox1988c++}. Porém, para a implementação dos algoritmos evolutivos, será usado o programa Matlab\textsuperscript{\textregistered}, que é uma linguagem alto nível para as operações de Álgebra Linear usadas em algoritmos evolutivos, conforme \cite{guide1998mathworks}.

Nesse contexto, surgem várias questões a serem resolvidas para se ter um time vencedor. 
